                  ...
                  <div class="col-sm-12">
                    <router-link :to="{ name: 'transaction', params: { hash: transaction.hash } }">
                      ...
                      <span v-else-if="transaction.message_id === 0 && transaction.body.from === keyPair.publicKey">
                        <strong v-numeral="transaction.body.amount"/>,
                        <strong v-numeral="transaction.body.amount_second"/>,
                        <strong v-numeral="transaction.body.amount_third"/> funds sent
                      </span>
              ...
              <form @submit.prevent="addFunds">
                <div class="form-group">
                  ...
                  <div v-for="variant in variantsSecond" class="form-check form-check-inline">
                    <input :id="variant.id" :value="variant.amount" :checked="amountToAddSecond == variant.amount" v-model="amountToAddSecond" class="form-check-input" type="radio">
                    <label :for="variant.id" class="form-check-label">${{ variant.amount }}</label>
                  </div>
                  <br>
            ...
            <div class="card-body">
              <form @submit.prevent="transfer">
                <div class="form-group">
                  ...
                  <input v-model="receiverToSecond" type="text" class="form-control" placeholder="Enter public key" required>
                  <input v-model="receiverToThird" type="text" class="form-control" placeholder="Enter public key" required>
                </div>
                <div class="form-group">
                  <label>Amount:</label>
                  <div class="input-group">
                    ...
                    <input v-model="amountToTransferSecond" type="number" class="form-control" placeholder="Enter amount" min="0" required>
                    <input v-model="amountToTransferThird" type="number" class="form-control" placeholder="Enter amount" min="0" required>
                  </div>
                </div>
  ...
  module.exports = {
    ...
    data() {
      return {
        ...
        balance: 0,
        amountToAdd: 10,
        amountToAddSecond: 10,
        amountToAddThird: 10,
        receiver: '',
        receiverToSecond: '',
        receiverToThird: '',
        amountToTransfer: '',
        amountToTransferSecond: '',
        amountToTransferThird: '',
    ...
    methods: {
      ...
      async addFunds() {
        ...
        try {
          const data = await this.$blockchain.addFunds(this.keyPair, this.amountToAdd, this.amountToAddSecond, this.amountToAddThird)
          this.balance = data.wallet.balance
          this.transactions = data.transactions
          this.isSpinnerVisible = false
      ...
      async transfer() {
        ...
        try {
          const data = await this.$blockchain.transfer(this.keyPair, this.receiver, this.receiverToSecond, this.receiverToThird, this.amountToTransfer, this.amountToTransferSecond, this.amountToTransferThird)
          this.balance = data.wallet.balance
          this.transactions = data.transactions
          this.isSpinnerVisible = false
