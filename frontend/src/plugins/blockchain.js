...
module.exports = {
  install(Vue) {
    Vue.prototype.$blockchain = {
      ...
      addFunds(keyPair, amountToAdd, amountToAddSecond, amountToAddThird) {
        const TxIssue = getTransaction(TX_ISSUE_ID)

        const data = {
          pub_key: keyPair.publicKey,
          amount: amountToAdd.toString(),
          amount_second: amountToAddSecond.toString(),
          amount_third: amountToAddThird.toString(),
      ...
      transfer(keyPair, receiver, receiver_to_second, receiver_to_third, amountToTransfer, amountToTransferSecond, amountToTransferThird) {
        const TxTransfer = getTransaction(TX_TRANSFER_ID)

        const data = {
          from: keyPair.publicKey,
          to: receiver,
          to_second: receiver_to_second,
          to_third: receiver_to_third,
          amount: amountToTransfer,
          amount_second: amountToTransferSecond,
          amount_third: amountToTransferThird,
          seed: Exonum.randomUint64(),
